<template>
  <div class="app-container" style="background-color: #F3F6F8">
   ...
   padding-bottom: 10px;margin-left: 200px;width: 1280px">
      <h3 style="margin-left: 20px">调拨修改基础信息</h3>
    ...
    margin-top: 20px;padding-bottom: 10px;margin-left: 200px;width: 1280px;height: 700px ">
      ...
      <div style="border-top: 1px solid #eeeeee;padding-top: 20px;padding-left: 30px">
        ...
        <el-table
          :loading="loading" style="margin-top: 50px"
          ...
          <el-table-column label="入库单价" prop="goods.wrPrice" :formatter="wrPrice" align="center"/>
          <el-table-column :formatter="computedTransferAccount" label="调拨金额" align="center" prop="transferAccount"
                           width="100px"/>
...
export default {
  ...
  data() {
    return {
      ...
      //入库仓库计划数量
      inNums: 0,
      //temp存储调拨明细对象  id   data
      originalTransferDetailsList:{},
  ...
  computed: {
    ...
    //调拨总数量
    totaltransferCount() {
      return this.formData.transferDetailsList?.reduce((accumulator, currentValue) => {
    ...
    totaltransferAccount() {
      const totalAmount = this.formData.transferDetailsList?.reduce((accumulator, currentValue) => {
  ...
  methods: {
    ...
    getDetailsByTid() {
      getTransfer(this.tid).then(response => {
        ...
        //获取出库id
        this.originalTransferDetailsList.id = this.formData.outWId;
        // 获取入库id
        this.originalTransferDetailsList.inId = this.formData.inWId;
        //封装出库仓库临时货品数据
        this.originalTransferDetailsList.datas = this.inventorysheetInf.selectGoods;
        //封装临时的入库库位
        this.originalTransferDetailsList.inlocations = this.inventorysheetInf.selectGoods.map(item=>item.inLocationId);
      });

    },
    // 计算调拨明细的金额
    computedTransferAccount(row) {
      const dbje =(row.goods.wrPrice || 0) * (row.quantity || 0);
      return dbje.toFixed(2);
    ...
    handleSelectChangeIn(e) {
      ...
      if (this.whIn?.w_id == this.formData.outWId) {
        this.$modal.msgWarning("入库仓库和出库仓库不能为同一仓库!");
        this.formData.inWId = null;
      } else {
        this.getSlName();
        if (e==this.originalTransferDetailsList.inId){
          for (let i = 0; i < this.inventorysheetInf.selectGoods.length; i++) {
            this.inventorysheetInf.selectGoods[i].inLocationId = this.originalTransferDetailsList.inlocations[i];
          }
        }else {
          this.inventorysheetInf.selectGoods=this.inventorysheetInf.selectGoods.filter(item=>{
            item.inLocationId=null;
            return item;
          })
        }
    ...
    handleSelectChangeOut(e) {
      ...
      if (this.whIn?.w_id == this.whOut?.w_id || this.whOut?.w_id==this.formData.inWId) {
        this.$modal.msgWarning("入库仓库和出库仓库不能为同一仓库!");
        this.formData.outWId = null;
      } else {
        // this.formData.transferDetailsList = [];

        if (this.originalTransferDetailsList.id == e){
          this.$set(this.inventorysheetInf, 'selectGoods', this.originalTransferDetailsList.datas);
        }else {
    ...
    //获取调入仓库所有库位
    getSlName() {
      console.log(this.formData.inWId)
      listSlName(this.formData.inWId).then(response => {
    ...
    addGoodsPage() {
      if (this.formData.outWId && this.formData.inWId) {
        ...
        this.getGoodsList();
        this.tempSelectGoodsList =this.inventorysheetInf.selectGoods //将回显调拨明细赋给已选中的货品
        console.log("open========"+this.tempSelectGoodsList[0].quantity)
      } else {
    ...
    //调拨数量失去焦点
    tquantityNums(row) {
      if (row.stock.itemQuantity < row.quantity) {
